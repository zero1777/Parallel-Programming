...
int main(int argc, char** argv) {
    ...
    auto readPngBegin = high_resolution_clock::now();

    read_png(argv[1], &src_img, &height, &width, &channels);
    assert(channels == 3);

    // Stop measuring time and calculate the elapsed time
    auto readPngEnd = high_resolution_clock::now();
    duration<double, std::milli> readPng = readPngEnd - readPngBegin;
    std::cout << "read_png execution time: " << readPng.count() << "ms" << std::endl;

    // Start measuring time
    auto mallocBegin = high_resolution_clock::now();

    unsigned char* dst_img =
        (unsigned char*)malloc(height * width * channels * sizeof(unsigned char));

    // Stop measuring time and calculate the elapsed time
    auto mallocEnd = high_resolution_clock::now();
    duration<double, std::milli> _malloc = mallocEnd - mallocBegin;
    std::cout << "malloc execution time: " << _malloc.count() << "ms" << std::endl;

    // cudaHostRegister(src_img, height * width * channels * sizeof(unsigned char), cudaHostRegisterDefault);
    ...
    // Start measuring time
    auto writePngBegin = high_resolution_clock::now();
    
    write_png(argv[2], dst_img, height, width, channels);
    
    // Stop measuring time and calculate the elapsed time
    auto writePngEnd = high_resolution_clock::now();
    duration<double, std::milli> writePng = writePngEnd - writePngBegin;
    std::cout << "write_png execution time: " << writePng.count() << "ms" << std::endl;

    

    // Start measuring time
    auto freeBegin = high_resolution_clock::now();

    free(src_img);
    free(dst_img);

    // Stop measuring time and calculate the elapsed time
    auto freeEnd = high_resolution_clock::now();
    duration<double, std::milli> _free = freeEnd - freeBegin;
    std::cout << "free execution time: " << _free.count() << "ms" << std::endl;

    // IO time
    std::cout << "I/O time: " << readPng.count() + writePng.count() << "ms" << std::endl;

    // CPU time
    std::cout << "CPU execution time: " << _malloc.count() + _free.count() << "ms" << std::endl;
