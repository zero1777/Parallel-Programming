...
string constructSolution(State st) {
    int prMove[4][2] = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    ...
    for (int i=st.seq.size()-1; i>=0; i--) {
        ...
        if (i == st.seq.size()-1 && !visitable) return "";
    }
...
int main(int argc, char *argv[]) {
    ...
    // tbb
    tbb::concurrent_queue<State> tqu;
    tbb::concurrent_unordered_set<State, StateHash> visited;
    // tbb::mutex m;

    // tbb
    tqu.push(State(initBoxPos, vector<Pair>()));
    ...
    // tbb
    while (!tqu.empty() && !cancel) {
        tbb::parallel_for(tbb::blocked_range<int>(0, tqu.unsafe_size()),
            ...
            for (int i=0; i<r.size(); i++) {
                if (!cancel) {
                    ...
                    if (tqu.try_pop(st)) {
                        // printMap(st);
                        if (!cancel && done(st)) {
                            string sol = constructSolution(st);
                            if (!cancel && sol != "") {
                                if (!cancel) finalState = sol;
                                cancel = true;
                            }
                        }
