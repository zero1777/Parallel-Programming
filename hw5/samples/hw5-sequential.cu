...
int main(int argc, char** argv) {
    ...
    std::vector<double> tqx, tqy, tqz, tvx, tvy, tvz;
    if (hit_time_step != -2) {
        read_input(argv[1], n, planet, asteroid, qx, qy, qz, vx, vy, vz, m, type);
        tqx = qx; tqy = qy; tqz = qz;
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int step = 0; step <= param::n_steps; step++) {
                ...
                double ddx = qx[planet] - qx[i];
                double ddy = qy[planet] - qy[i];
                double ddz = qz[planet] - qz[i];
                if (!getCost) {
                    if (ddx * ddx + ddy * ddy + ddz * ddz < (step) * param::dt * param::missile_speed * (step) * param::dt * param::missile_speed) {
                        getCost = true;
                        m[i] = 0;
            ...
            if (!isHit) {
                if (missile_cost == 0 || missile_cost > cost) {
                    missile_cost = cost;
                    gravity_device_id = i;
                }
            }
