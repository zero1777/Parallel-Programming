...
int main(int argc, char** argv) {
    ...
    int avgHeight = (height + mpiSize - 1) / mpiSize;

    unsigned char *ourImage = new unsigned char[avgHeight * width * 4];
    unsigned char *collImage = new unsigned char[avgHeight * width * mpiSize * 4];
    ...
    for (int i = rank; i < height; i += mpiSize) {
        for (int j = 0; j < width; j++) {
            ...
            // mpi
            ourImage[(i-rank) / mpiSize * width * 4 + j * 4 + 0] = (unsigned char)fcol.r;  // r 
            ourImage[(i-rank) / mpiSize * width * 4 + j * 4 + 1] = (unsigned char)fcol.g;  // g 
            ourImage[(i-rank) / mpiSize * width * 4 + j * 4 + 2] = (unsigned char)fcol.b;  // b 
            ourImage[(i-rank) / mpiSize * width * 4 + j * 4 + 3] = 255;                    // a 
    ...
    for (int i=0; i<mpiSize; i++) {
        recvcnt[i] = avgHeight * width * 4;
        displs[i] = i * avgHeight * width * 4;
    }

    MPI_Gatherv(ourImage, avgHeight * width * 4, MPI_UNSIGNED_CHAR, collImage, recvcnt, displs, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        for (int i=0; i<height; i++) {
            for (int j=0; j<width; j++) {
                int whichRank = i % mpiSize;
                int whichIdx = i / mpiSize;

                image[i][4 * j + 0] = collImage[whichRank * avgHeight * width * 4 + whichIdx * width * 4 + j * 4 + 0];
                image[i][4 * j + 1] = collImage[whichRank * avgHeight * width * 4 + whichIdx * width * 4 + j * 4 + 1];
                image[i][4 * j + 2] = collImage[whichRank * avgHeight * width * 4 + whichIdx * width * 4 + j * 4 + 2];
                image[i][4 * j + 3] = collImage[whichRank * avgHeight * width * 4 + whichIdx * width * 4 + j * 4 + 3];         
            }
        }
    }
