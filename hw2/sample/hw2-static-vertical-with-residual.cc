...
int main(int argc, char** argv) {
    ...
    // mpi
    // fast ceiling
    int avgWidth = (width + mpiSize - 1) / mpiSize;
    unsigned char *ourImage = new unsigned char[height * avgWidth * 4];
    unsigned char *collImage = new unsigned char[height * avgWidth * mpiSize * 4];

    int residual = width % mpiSize;
    int maxWidth = width - residual;
    int totalRes = residual * height;
    ...
    // mpi
    // constant variables that dont need to calculate in loop
    // ---create camera
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = rank; j < maxWidth; j += mpiSize) {
            ...
            // mpi
            ourImage[i * avgWidth * 4 + (j-rank) / mpiSize * 4 + 0] = (unsigned char)fcol.r;  // r 
            ourImage[i * avgWidth * 4 + (j-rank) / mpiSize * 4 + 1] = (unsigned char)fcol.g;  // g 
            ourImage[i * avgWidth * 4 + (j-rank) / mpiSize * 4 + 2] = (unsigned char)fcol.b;  // b 
            ourImage[i * avgWidth * 4 + (j-rank) / mpiSize * 4 + 3] = 255;                    // a 
    ...
    // equally distributing the residual to each process
    if (width != maxWidth) {
        #pragma omp parallel for schedule(dynamic)
        for (int res=rank; res < totalRes; res += mpiSize) {
            int i = res / residual;
            int j = (res % residual) + maxWidth;
            ...
            // mpi
            ourImage[i * avgWidth * 4 + (avgWidth-1) * 4 + 0] = (unsigned char)fcol.r;  // r 
            ourImage[i * avgWidth * 4 + (avgWidth-1) * 4 + 1] = (unsigned char)fcol.g;  // g 
            ourImage[i * avgWidth * 4 + (avgWidth-1) * 4 + 2] = (unsigned char)fcol.b;  // b 
            ourImage[i * avgWidth * 4 + (avgWidth-1) * 4 + 3] = 255;                    // a 
    ...
    for (int i=0; i<mpiSize; i++) {
        recvcnt[i] = height * avgWidth * 4;
        displs[i] = i * height * avgWidth * 4;
    }

    MPI_Gatherv(ourImage, height * avgWidth * 4, MPI_UNSIGNED_CHAR, collImage, recvcnt, displs, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        for (int i=0; i<height; i++) {
            for (int j=0; j<maxWidth; j++) {
                int whichRank = j % mpiSize;
                int whichIdx = j / mpiSize;

                image[i][4 * j + 0] = collImage[whichRank * height * avgWidth * 4 + i * avgWidth * 4 + 4 * whichIdx + 0];
                image[i][4 * j + 1] = collImage[whichRank * height * avgWidth * 4 + i * avgWidth * 4 + 4 * whichIdx + 1];
                image[i][4 * j + 2] = collImage[whichRank * height * avgWidth * 4 + i * avgWidth * 4 + 4 * whichIdx + 2];
                image[i][4 * j + 3] = collImage[whichRank * height * avgWidth * 4 + i * avgWidth * 4 + 4 * whichIdx + 3];         
            }
        }

        if (maxWidth != width) {
            #pragma omp parallel for
            for (int res=0; res < totalRes; res++) {
                int i = res / residual;
                int j = (res % residual) + maxWidth;
                int whichRank = res % mpiSize; 
                int whichIdx = avgWidth - 1;
